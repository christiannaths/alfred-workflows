...
if ARGV[0] == 'ip'
	...
	feedback.add_item({:title => "External IP: #{external_ip}", :subtitle => 'Press Enter to copy', :arg => external_ip})
	ips.each do |interface|
		ip = interface[:ip]
		name = interface[:name]
		device = interface[:interface]
		feedback.add_item({:title => "#{name} (#{device}) IP: #{ip}", :subtitle => 'Press Enter to copy', :arg => ip})
	end
else
	macs.each do |interface|
		...
		if !mac.nil? && mac.length > 0
			feedback.add_item({:title => "#{name} (#{device}) MAC: #{mac}", :subtitle => 'Press Enter to copy', :arg => mac})
		end
	end
end
